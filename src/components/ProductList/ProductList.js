            ...
            {_.map(_.get(rowData, 'collaborators'), (item, email) => (
              <li key={email}>
                ...
                {_.get(item, 'isCurrentAccount') ? (
                  <span className={cx(s.label, s.labelSuccess)}>it's you</span>
                ) : null}
              </li>
            ))}
          </ul>
    ...
    return (
      <div className={s.root}>
        <div className={s.container}>
          <Breadcrumb>
            <Breadcrumb.Item active>App list</Breadcrumb.Item>
          </Breadcrumb>
          <Col style={{ marginBottom: '20px' }}>
            <Button onClick={this.props.popAddApp} bsStyle="primary">
              Add app
            </Button>
          </Col>
          <Table striped bordered condensed hover responsive>
            <thead>
              <tr>
                <th style={{ textAlign: 'center' }}>App name</th>
            ...
            <tbody>
              {this.props.rs.length > 0 ? (
                _.map(this.props.rs, (item, index) =>
