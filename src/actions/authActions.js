...
export function fetchAccessKeys() {
  return dispatch => {
    ...
    return restApi.getAccessKeys().then(data => {
...
export function reomveAccessKey(name) {
  return dispatch => {
    ...
    return restApi.removeAccessKey(name).then(data => {
      if (_.get(data, 'status') !== 'OK') {
        dispatch(
          addShowMsg(
            'Delete key failed:' + _.get(data, 'errorMessage'),
            'danger'
          )
        );
      } else {
        dispatch(addShowMsg('Delete key successfully:' + name, 'success'));
      }
...
export function patchAccessKey(name, friendlyName = null, ttl = 0) {
  return dispatch => {
    ...
    return restApi
      .patchAccessKey(name, friendlyName, ttl)
      .then(data => {
        if (_.get(data, 'status') !== 'OK') {
          dispatch(
            addShowMsg(
              'Modify key failed:' + _.get(data, 'errorMessage'),
              'danger'
            )
          );
        } else {
          dispatch(
            addShowMsg('Modify key successfully:' + friendlyName, 'success')
          );
        }
...
export function createAccessKey() {
  return dispatch => {
    ...
    return restApi.createAccessKey().then(data => {
      if (_.get(data, 'status') !== 'OK') {
        dispatch(
          addShowMsg(
            'Create key failed:' + _.get(data, 'errorMessage'),
            'danger'
          )
        );
      } else {
        dispatch(
          addShowMsg(
            'Create key succesfully:' +
              _.get(data, 'results.accessKey.friendlyName'),
            'success'
          )
        );
      }
